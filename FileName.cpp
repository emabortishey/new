...
int main()
{
	...
	// перменные выбора, а также буферы для работы со старой версией меню (+ счётчик обходов программы)

	int choice;
	int num, den, bufn, bufd, count = 0;

	// инициализация и обьявление первичных значений для обоих обьектов класа

	Fraction main{ 1, 8 };
	Fraction users{ 2, 8};

	// запрос первой дроби у пользователя
	...
	do {
		...
		// проверка первый ли это обход меню у программы и запрос второй дроби
		...
		// увеличение количества совершенных обходов при этом запуске
		count++;
